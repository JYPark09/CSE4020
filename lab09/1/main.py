...
def render():
    ...
    alpha, beta, gamma = map(np.radians, gObjRotation)

    M = np.identity(4)
    R1 = np.array([[np.cos(alpha), -np.sin(alpha), 0],
                   [np.sin(alpha), np.cos(alpha), 0], [0, 0, 1]])
    R2 = np.array([[1, 0, 0], [0, np.cos(beta), -np.sin(beta)],
...
def key_callback(window, key, scancode, action, mods):
    if action == glfw.PRESS or action == glfw.REPEAT:
        if key == glfw.KEY_A:
            ...
            gObjRotation[0], gObjRotation[1], gObjRotation[2] = 0, 0, 0
