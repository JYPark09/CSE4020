...
def render(t):
    global gCamAng, gCamHeight, gPrevMat
    ...
    curpos = (J2 @ T1 @ np.array([0, 0, 0, 1]))[:-1]
    oldpos = (gPrevMat @ T1 @ np.array([0, 0, 0, 1]))[:-1]
    velocity = curpos - oldpos

    glBegin(GL_LINES)
    glColor3ub(255, 0, 0)
    glVertex3fv(curpos)
    glVertex3fv(curpos + 10 * velocity)
    glEnd()
